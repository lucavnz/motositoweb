...
async function main() {
    console.log("Inizializzo generazione AI per KTM Nuove...");

    const brands = await client.fetch(`*[_type == "brand" && name match "KTM"]{_id, name}`);
    if (brands.length === 0) {
        console.error("❌ Marchio KTM non trovato in Sanity.");
        return;
    }
    const ktmId = brands[0]._id;
    console.log(`✅ Trovato marchio KTM (ID: ${ktmId})`);

    // Fetch KTM motorcycles that are NEW and DO NOT have an AI generated description
    ...
    console.log(`🏍 Trovate ${bikes.length} moto KTM nuove che necessitano di generazione AI.`);
    ...
    for (let i = 0; i < bikes.length; i++) {
        ...
        console.log(`\n⏳ Generazione in corso per: KTM ${bike.model} (${bike.year}) [ID: ${bike._id}]`);
        ...
        let success = false;
        try {
            const prompt = `Cerca informazioni dettagliate sulla moto KTM ${bike.model} del ${bike.year}.
            ...
            console.log(`⏩ Passo subito al prossimo modello/chiave per rirpovare la stessa moto...`);
        }
    ...
    console.log(`\n🎉 Processo completato! Generate con successo ${successCount}/${bikes.length} descrizioni per KTM.`);
}
